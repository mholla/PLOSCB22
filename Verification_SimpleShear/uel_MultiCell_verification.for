...
!
! Shuolun Wang, April 2020 @ND 
! 
! version4 includes 3d element 
! 3DBarMultiFieldDeform_v4.inp
! 3DBallMultiFieldDeform_v4.inp
***********************************************************************
!
...
C     must be set equal to or greater than 15.
      ...
      uvar(11) = globalSdv(noel-ElemOffset,npt,11) ! T12
      uvar(12) = globalSdv(noel-ElemOffset,npt,12) ! Tdiff
...
*
     ...
     4 PREDEF(2,NPREDF,NNODE),LFLAGS(*),JPROPS(*)
      ...
      real*8 coordintC(nDim,1),coordint(nDim,1),velocity
...
c         call integ(props,nprops,dtime,F_tau,F_t,c_tau,dcdX,coordint,
c     +              qflux,source,velocity,Diffusivity,dqdc,
c     +              T_tau,dJdt,SpTanMod,SpUCMod,SpCUMod
c     +              )

c         call integ(props,nprops,dtime,F_tau,F_t,c_tau,dcdX,coordintC,
c     +              coordint,qflux,source,velocity,Diffusivity,dqdc,
c     +              T_tau,dJdt,SpTanMod,SpUCMod,SpCUMod,mu,k_normal,k_para,
c     +              Je_tau,detFg,EffStretch,EffStretche,TrT) 

         call integ(props,nprops,TIME,dtime,F_tau,F_t,c_t,c_tau,dcdX,coordintC,
     +              coordint,qflux,source,velocity,Diffusivity,dqdc,
     +              T_tau,dJdt,SpTanMod,SpUCMod,SpCUMod,mu,k_normal,k_para,
     +              Je_tau,detFg,EffStretch,EffStretche,TrT,scalar_term)  
...
*
     ...
     4 PREDEF(2,NPREDF,NNODE),LFLAGS(*),JPROPS(*)
      ...
      real*8 DegreeX
      real*8 coordintC(nDim,1),coordint(nDim,1),qflux(nDim,1),source
      real*8 velocity,Diffusivity,dqdc(nDim,1),dJdt,SpUCMod(3,3)
      real*8 SpCUMod(3,3,3),mu,k_normal,k_para,Je_tau,detFg
      real*8 EffStretch,EffStretche,TrT,scalar_term
      real*8 AmatUC(6,1),AmatCU(3,9)
      real*8 T12,T11,T33,Tdiff
...
c      enddo
      ...
      do intpt=1,nIntPt
         ...
         ! Obtain the concentration of riboflavin and its derivative's at 
         !  this intPt at the begining and end of the incrment
         !
         c_tau = zero
         c_t = zero
...
c         write(*,*),'check before integ'
         ...
         ! Perform the constitutive time integration at this integ. point 
         ! 
c         call integ(props,nprops,TIME,dtime,F_tau,F_t,c_t,c_tau,dcdX,coordintC,
c     +              coordint,qflux,source,velocity,Diffusivity,dqdc,
c     +              T_tau,dJdt,SpTanMod,SpUCMod,SpCUMod,mu,k_normal,k_para,
c     +              Je_tau,detFg,EffStretch,EffStretche,TrT,scalar_term)  
c         call integ(props,nprops,TIME,dtime,F_tau,F_t,c_t,c_tau,dcdX,coordintC,
c     +              coordint,qflux,source,velocity,Diffusivity,dqdc,
c     +              T_tau,dJdt,SpTanMod,SpUCMod,SpCUMod,mu,k_normal,k_para,
c     +              Je_tau,detFg,EffStretch,EffStretche,TrT,scalar_term,
...
c         write(*,*),'T_tau=',T_tau
         ...
         globalSdv(jelem,intPt,9) = EffStretche !  Effective elastic Stretch            
         globalSdv(jelem,intPt,10) = TrT !  trace of Cauchy stress  
         globalSdv(jelem,intPt,11) = T12
         globalSdv(jelem,intPt,12) = Tdiff
...
************************************************************************ 
      subroutine integ(props,nprops,TIME,dtime,F_tau,F_t,c_t,c_tau,dcdX,coordintC,
     +              coordint,qflux,source,velocity,Diffusivity,dqdc,
     +              T_tau,dJdt,SpTanMod,SpUCMod,SpCUMod,mu,k_normal,k_para,
     +              Je_tau,detFg,EffStretch,EffStretche,TrT,scalar_term)   
      ...
      real*8 Finv(3,3),TIME,SpUIMod(3,3),SpIUmod(3,3),Bdis(3,3)
      ...
      real*8 alpha,alpha_Gc,alpha_v,Gc,vel
      ...
      real*8 TrT
      real*8 scalar_term(1,4),c_t
      real*8 Gc_time1,epsilon1
     ...
     +     half=1.d0/2.d0,nine=9.d0,four=4.d0)
      ...
      alpha_Gc      = props(2) ! smooth parameter for source term 
      alpha_v       = props(3) ! smooth parameter for velocity
      alpha_d       = props(4) ! smooth parameter for diffusicity 
      alpha_mu      = props(5) ! smooth parameter for stiffness 
      alpha_k       = props(6) ! smooth parameter for growth parameter
      Gc            = props(7) ! base value for source term
      vel           = props(8) ! base value for velocity
      diff          = props(9) ! diffusivity
      c0            = props(10) ! cell migration threshold 
      k_s           = props(11) ! shape parameter for growth 
      k_c_normal    = props(12) ! shape parameter for growth
      k_c_para      = props(13) ! shape parameter for growth      
      alpha_normal  = props(14) ! shape parameter for growth
      alpha_para    = props(15) ! shape parameter for growth
      lambda_c      = props(16) ! Lame constant 
      lambda_s      = props(17) ! Lame constant 
      mu_c          = props(18) !Lame constant
      mu_s          = props(19) !Lame constant
      epsilon1      = props(20) ! 
...
c      nz = 0.0
      ...
      ! 1-D 
c      coordiff = -(xcoord - 0.7)
      ! 2-D
c      coordiff = -(radius - 0.95)
      ! 3-D ball
      coordiff = -(radius_3d - 1.0)
      ...
      ! 1-D 
c      coordiff = (xcoord - 0.98)
      ! 2-D 
c      coordiff = (radius - 0.98)
      ! 3-D
      coordiff = (radius_3d - 0.98)
      call Hhat(coordiff,alpha_mu,Hhatmu) ! Heaveistep function for stiffness
      call Hhat(coordiff,alpha_k,Hhatk) ! Heaveistep function for growth parameter

      ! 1-D
c      coordiff = -(xcoord - 0.2)
      ! 2-D 
c      coordiff = -(radius - 0.2)     
      ! 3-D
      coordiff = -(radius_3d - 0.2) 
...
c      Diffusivity = diff*(1.5 - 0.95*Hhatd)
      ...
      Gc_time1 = (epsilon1**2.0)/((TIME - 100.0)**2.0 + epsilon1**2.0)

c      source = Gc_time1*Gc*Hhats
      source = Gc*Hhats
      ...
      ! stiffness 
      ! 
c      mu = mu_s + (mu_c - mu_s)*Hhatmu
c      lambda = lambda_s + (lambda_c - lambda_s)*Hhatmu
      ...
      ! k values 
      ! 
      k_normal = k_s + (k_c_normal - k_s)*Hhatk
c      k_normal = 0.0
      k_para = k_s + (k_c_para - k_s)*Hhatk
c      k_para = k_s
      
      ! growth paraemter
      !  
      theta_normal  = (1.0 + k_normal*c_tau)**alpha_normal
      theta_para    = (1.0 + k_para*c_tau)**alpha_para
...
c      SpCUMod = - dqdFFT
      ...
      ! calcualte scalar_term 
      ! 
      scalar_term = zero    
      scalar_term(1,1) = (c_tau - c_t)/dtime + c_tau/dtime + source
      scalar_term(1,2) = (c_tau - c_t)/dtime + c_tau/dtime + source
      scalar_term(1,3) = (c_tau - c_t)/dtime + c_tau/dtime + source
      scalar_term(1,4) = (c_tau - c_t)/dtime + c_tau/dtime + source
     ...
     +            )
      ...
      fac4 = k_normal*alpha_normal*(1.0 + k_normal*c_tau)**(alpha_normal-1.0)
      fac5 = k_para*alpha_para*(1.0 + k_para*c_tau)**(alpha_para-1.0)
